 ...
 * - 賽璐璐材質:3 階色帶 + inverted-hull 描邊 + rim(T4 定案;T1 曾以 2 階驗證「能變暗」)
 */
...
// 正統賽璐璐的 rim light(邊緣逆光):加在 emissive 通道,只亮不暗(不破壞色階)
...
// rim 的共用 uniform(全髮束同值;非 per-束狀態,一顆就夠)
const uRimColor: THREE.IUniform<THREE.Color> = {
  value: new THREE.Color(RIM_COLOR).multiplyScalar(RIM_STRENGTH),
};
const uRimPower: THREE.IUniform<number> = { value: RIM_POWER };

/**
 * 把材質改造成可彎曲(+可選 rim):
 * - 頂點:spineT 屬性與 uBend uniform,沿 uBend 位移、spineT² 加權(髮根不動、
 *   髮尖全量)— M2 動態的共用基建。uBend 由呼叫端提供:同一根髮束的髮體與
 *   描邊殼共用一顆,彎曲永遠同步。
 * - 片段(rim=true,只給 toon 髮體):邊緣逆光加進 totalEmissiveRadiance —
 *   只加亮,不動賽璐璐色階的暗部。
 */
function makeBendable(mat: THREE.Material, uBend: THREE.IUniform<THREE.Vector3>, rim = false): void {
    ...
    if (rim) {
      ...
      shader.fragmentShader = shader.fragmentShader
        ...
        .replace(
          ...
          `#include <emissivemap_fragment>
  float rimFacing = 1.0 - saturate(normalize(normal).z);
  totalEmissiveRadiance += uRimColor * pow(rimFacing, uRimPower);`,
        );
    }
  };
  // 正交相機下視向恆為 view-space +z,rim 直接用 normal.z(normalize(vViewPosition)
  // 在像素座標正交投影裡是隨螢幕位置亂指的向量,three 自家 chunk 也特判 isOrthographic)。
  // rim 是 onBeforeCompile 閉包的一部分但不改變 toString → 顯式 cache key 防止
  // 同型材質 rim 有無混用同一支 program
...
export function createHair3D(insertBefore: HTMLElement): Hair3D {
  ...
  SPIKES.forEach((s, i) => {
    ...
    makeBendable(spikeMat, uBend, true); // 髮體帶 rim
    makeBendable(hullMat, uBend); // 描邊殼不帶(BackSide 的 rim 沒有意義)
