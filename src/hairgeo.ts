...
// 兩特徵:主體是往上後方的大尖刺簇(ez 負)、左右不對稱(左側整體偏高、主尖偏左)。
// ⚠ tilt 的正負代表「往外」還是「往內」**不是全域慣例** —— 它是繞局部 z 軸的 roll,
// 而局部 x 軸的像跟著圓頂法線轉,所以同一個號在頭頂與在帽簷是相反的方向。弄反的
// 症狀是髮束跨過中線互相交叉、描邊疊成一塊深色(實機可見,紙上看不出來)。
// 帽簷那圈由測試釘住:見 hairgeo.test.ts「帽簷髮束往外倒」。
export const SPIKES: SpikePlacement[] = [
  // ---- 帽簷(brim):足印半徑 ~0.95,髮根落在接近赤道處 —— 髮束因此從頭皮側面長出,
  // 把髮際線那一圈真髮蓋住。少了這圈,髮束全從頭頂冒出來,讀起來是戴皇冠不是頭髮。
  // tilt 由數值解出:在「往外」的前提下讓生長方向的向上分量到 0.8 —— 全部往上走,
  // 只是越靠外側越往外傾。手調容易掉進兩個極端(太小=從側面平伸、太大=翻過去往下垂)。
  { ex: -0.95, ez: 0.16, h: 0.66, tilt: -0.62, bend: -0.3, r: 0.12 },
  { ex: -0.8, ez: 0.52, h: 0.58, tilt: -0.82, bend: -0.26, r: 0.1 },
  { ex: -0.93, ez: -0.28, h: 0.7, tilt: -0.69, bend: -0.28, r: 0.12 },
  { ex: -0.78, ez: -0.58, h: 0.6, tilt: -0.93, bend: -0.24, r: 0.11 },
  { ex: 0.95, ez: 0.16, h: 0.6, tilt: 0.62, bend: 0.3, r: 0.11 },
  { ex: 0.8, ez: 0.52, h: 0.54, tilt: 0.82, bend: 0.26, r: 0.1 },
  { ex: 0.93, ez: -0.28, h: 0.64, tilt: 0.69, bend: 0.28, r: 0.11 },
  { ex: 0.78, ez: -0.58, h: 0.56, tilt: 0.93, bend: 0.24, r: 0.1 },
  { ex: -0.36, ez: -0.9, h: 0.46, tilt: -0.35, bend: -0.16, r: 0.09 },
  { ex: 0.34, ez: -0.9, h: 0.44, tilt: 0.33, bend: 0.16, r: 0.09 },
  // ---- 前額帽簷:足印 0.87~0.95,補住額頭正前方那一段髮際線。
  // 這裡的法線幾乎朝正前方,所以髮束是「朝鏡頭斜上方伸」——從正面看是實心的錐體
  // (髮尖朝鏡頭),不是舊版垂掛髮束那種底蓋正對鏡頭的深色圓洞。
  // 少了這排,額頭上會露出一整條沒被蓋住的染金真髮(實機可見)。
  { ex: -0.44, ez: 0.87, h: 0.55, tilt: -0.38, bend: -0.2, r: 0.12 },
  { ex: -0.15, ez: 0.95, h: 0.48, tilt: -0.18, bend: -0.14, r: 0.11 },
  { ex: 0.15, ez: 0.95, h: 0.46, tilt: 0.18, bend: 0.14, r: 0.11 },
  { ex: 0.44, ez: 0.87, h: 0.52, tilt: 0.38, bend: 0.2, r: 0.12 },
  // ---- 前額中段:補住「前額帽簷的髮尖」與「主體那排的髮根」之間的空檔。
  // 少了這圈,足印 0.87 → 0.16 之間有 0.71 的斷層(中間只有兩側的帽簷 0.52),
  // 畫面上是額頭正中一條橫向空白(實機可見)。
  { ex: -0.6, ez: 0.62, h: 0.72, tilt: -0.6, bend: -0.24, r: 0.13 },
  { ex: -0.22, ez: 0.62, h: 0.9, tilt: -0.05, bend: -0.16, r: 0.14 },
  { ex: 0.18, ez: 0.62, h: 0.85, tilt: 0.06, bend: 0.16, r: 0.14 },
  { ex: 0.55, ez: 0.62, h: 0.68, tilt: 0.54, bend: 0.22, r: 0.12 },
  // ---- 主體:頭頂往上後方的大尖刺簇。左右不對稱(主尖偏左且最高)是造型基準的錨點
 ...
 * 有了它,「這撮到底往哪長」才驗得起來,不必等實機照片才發現長反了。
 *
 * 可達方向只有 n 與 u(局部 x 軸的像,恆垂直於 n)的組合 —— 這是模型的硬限制:
 * 法線朝前上方的髮根做不出「往下垂又朝前」,roll 過 90° 一定往後倒。
 */
export function spikeGrowth(n: { x: number; y: number; z: number }, tilt: number):
  { x: number; y: number; z: number } {
  // u = R(UP→n)·e_x,UP=(0,-1,0)。Rodrigues 對 e_x 化簡後(s² = n.x²+n.z² = 1-n.y²,
  // 且 (1+n.y)/s² = 1/(1-n.y))收成三個分量,已是單位長 —— 旋轉不改變長度
  const den = 1 - n.y;
  const u = den < 1e-9
    ? { x: 1, y: 0, z: 0 } // n = -UP 的退化:局部 x 就是世界 x
    : { x: -n.y + (n.z * n.z) / den, y: n.x, z: (-n.x * n.z) / den };
  const st = Math.sin(tilt);
  const ct = Math.cos(tilt);
  return { x: -st * u.x + ct * n.x, y: -st * u.y + ct * n.y, z: -st * u.z + ct * n.z };
}
