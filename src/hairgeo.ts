...
// 三特徵:額前兩撮從髮際線往前外側揚起、主體往上後方的大尖刺簇(ez 負)、
// 左右不對稱(左側整體偏高、主尖偏左)。bend 與 tilt 同號=往外勾。
export const SPIKES: SpikePlacement[] = [
  // ---- 帽簷(brim):足印半徑 ~0.95,髮根落在接近赤道處,tilt 接近 π/2 讓生長方向
  // 幾乎是「側向」→ 髮束貼著頭皮往下外側倒,把髮際線那一圈真髮蓋住。
  // 少了這一圈,髮束全從頭頂往上長,讀起來是戴皇冠而不是頭髮(實機照片的主要問題)。
  { ex: -0.95, ez: 0.16, h: 0.66, tilt: -1.25, bend: -0.42, r: 0.12 },
  { ex: -0.8, ez: 0.52, h: 0.58, tilt: -1.15, bend: -0.34, r: 0.1 },
  { ex: -0.93, ez: -0.28, h: 0.7, tilt: -1.2, bend: -0.38, r: 0.12 },
  { ex: -0.78, ez: -0.58, h: 0.6, tilt: -1.1, bend: -0.32, r: 0.11 },
  { ex: 0.95, ez: 0.16, h: 0.6, tilt: 1.25, bend: 0.42, r: 0.11 },
  { ex: 0.8, ez: 0.52, h: 0.54, tilt: 1.15, bend: 0.34, r: 0.1 },
  { ex: 0.93, ez: -0.28, h: 0.64, tilt: 1.2, bend: 0.38, r: 0.11 },
  { ex: 0.78, ez: -0.58, h: 0.56, tilt: 1.1, bend: 0.32, r: 0.1 },
  { ex: -0.36, ez: -0.9, h: 0.46, tilt: -0.9, bend: -0.24, r: 0.09 },
  { ex: 0.34, ez: -0.9, h: 0.44, tilt: 0.9, bend: 0.24, r: 0.09 },
  // ---- 髮際線兩撮:短、細,往前外側揚起(悟空 SSJ1 的瀏海是揚的,不是垂的)。
  // ⚠ 不要改回 |tilt|>π/2 的「垂掛」—— tilt 是繞局部 z 的 roll,可達方向只有
  // 「圓頂法線」與「側向」的組合;髮際線那點的法線已朝前上方,roll 過 90° 必定
  // 往後倒進頭骨裡,畫面上只剩底蓋正對鏡頭的深色圓洞(圓頂錨點校準後實機可見)。
  // ⚠ tilt 的正負代表「往外/往內」**不是全域慣例**,會隨髮根在圓頂上的位置翻轉
  // (局部 x 軸的像跟著法線轉)。弄反會讓兩撮跨過中線交叉,描邊疊成額頭正中一個
  // 深色 X(實機可見)。有測試釘住:見 hairgeo.test.ts「髮際線兩撮往外長」。
  { ex: -0.3, ez: 0.88, h: 0.46, tilt: 1.0, bend: 0.2, r: 0.08 },
  { ex: 0.27, ez: 0.88, h: 0.42, tilt: -1.05, bend: -0.18, r: 0.075 },
