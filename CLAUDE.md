...
- **MediaPipe versions move in lockstep:** `package.json` pins `@mediapipe/tasks-vision` exactly, and `WASM_URL` (defined once in `detector.ts`, imported by `segmenter.ts`) hardcodes the same version on the CDN. Bump both together or the JS loader and WASM runtime skew.
- **Hair segmentation (`segmenter.ts`) is garnish, not core:** its model load is non-fatal (boot line shows SKIP and the app runs without gold hair), and inference is gated to the `ssj` phase only — never add per-frame segmentation outside that window without measuring on a phone (`?debug` shows per-segment cost as `hair ... last Nms`).
